...
* それぞれの$\bf x$に対する$t$の不確かさを表すために予測分布$p(t|\bf x \rm)$をモデル化

* 損失関数の期待値を最小にするように、任意の$\bf x$の新たな値に対する$t$を予測できる
...
入力空間が高次元の問題には実用的とは言えないが、

解析に的に扱いやすく洗練されたモデルの基礎をなす重要なモデル
...
<div style='text-align:center;'>
  <img src="/Users/marie/NAIST/PRML/prmlfigs-jpg/Figure3.1a.jpg" alt="Figure3.1a" style="zoom:30%"/>
  <img src="/Users/marie/NAIST/PRML/prmlfigs-jpg/Figure3.1b.jpg" alt="Figure3.1b" style="zoom:30%"/>
  <img src="/Users/marie/NAIST/PRML/prmlfigs-jpg/Figure3.1c.jpg" alt="Figure3.1c" style="zoom:30%"/>
  <p>
    Fig 3.1 基底関数の例<br>
    左：多項式、中央：ガウス基底関数、右：シグモイド基底関数
  </p>
</div>
...
$$
p({\sf t}|{\bf X},{\bf w},\beta) = \prod_{n=1}^N\mathcal{N}(t_n|{\bf w}^T\phi({\bf x}_n),\beta^{-1}). \tag{3.10}
$$

* 入力${\bf X}=\{{\bf x}_1,…,{\bf x}_N\}$
* 目標値$t_1,...,t_N$からなるデータ集合
* 列ベクトル$\sf t$：目標変数$\{t_n\}$をまとめたもの
...
* 最尤推定により${\bf w}と$$\beta$を決定できる

  * $\bf w$に関して最大化
  * 条件付きガウスノイズ分布の下での線形モデルに対する尤度関数の最大化は、二乗和誤差関数$E_D({\bf w})$ を最小化にすることと等価

* ノイズの精度パラメータ$\beta$に関しても最大化することができる
  $$
